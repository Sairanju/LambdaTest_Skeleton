...
if (!capabilities["LT:Options"].user || !capabilities["LT:Options"].accessKey) {
  throw new Error("LambdaTest Username or AccessKey is not set in the environment variables.");
}
...
const getErrorMessage = (obj, keys) =>
  keys.reduce((acc, key) => (typeof acc === "object" ? acc[key] : undefined), obj);
...
const test = base.test.extend({
  page: async ({ page, playwright }, use, testInfo) => {
    ...
    if (testInfo.project.name.match(/lambdatest/)) {
      ...
      // Run tests locally in case of local config provided
      await use(page);
    }
  },
});
